...
//if (c > 100) break;
	...
	var linkEmailGo = function(text, tagfill){

		var atom = '[^()<>@,;:\\\\\".\\[\\]\\x00-\\x20\\x7f]+'; // from RFC822
		var multi_atom = '('+atom+'(\\.'+atom+')*)';

		var atoms_start = new RegExp( '^'+multi_atom );
		var atoms_end = new RegExp( multi_atom+'$' );
		...
		while ((cursor < text.length) && loop){
			...
			if (pos === -1){
				...
				pre = text.substr(cursor, pos-cursor);
				hit = text.substr(pos, 1);
				post = text.substr(pos + 1);
			...
			if (ok){
				var matches = pre.match( atoms_end );
				if (matches){
					...
					hit = pre.substr(plen-len) + hit;
					pre = pre.substr(0, plen-len);
			...
			if (ok){
				var matches = post.match( atoms_start );
				if (matches){
	...
	autolink.prototype.linkEmail = function(str){
		...
		str = linkEmailGo(str, this.tagFill);
